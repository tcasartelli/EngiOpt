...
Components:
    ...
    - SNEncoder2D: Spectrally normalized convolutional encoder for 2D designs
...
class SNEncoder2D(nn.Module):
    """Spectrally normalized convolutional encoder for 2D designs.

    Identical to Encoder2D but with spectral_norm applied to every Conv2d layer,
    including the final to_latent projection. This constrains the Lipschitz constant
    of the encoder, making the latent mapping smoother and more robust to unseen inputs.

    Architecture: Input -> SN-Conv layers -> Latent vector
    - Input   [100x100]
    - SNConv1 [50x50]   (k=4, s=2, p=1)
    - SNConv2 [25x25]   (k=4, s=2, p=1)
    - SNConv3 [13x13]   (k=3, s=2, p=1)
    - SNConv4 [7x7]     (k=3, s=2, p=1)
    - SNConv5 [1x1]     (k=7, s=1, p=0)
    ...
    def __init__(
        ...
        self.features = nn.Sequential(
            spectral_norm(nn.Conv2d(1, 64, kernel_size=4, stride=2, padding=1, bias=False)),  # 100->50
            nn.BatchNorm2d(64),
            nn.LeakyReLU(0.2, inplace=True),
            spectral_norm(nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1, bias=False)),  # 50->25
            nn.BatchNorm2d(128),
            nn.LeakyReLU(0.2, inplace=True),
            spectral_norm(nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1, bias=False)),  # 25->13
            nn.BatchNorm2d(256),
            nn.LeakyReLU(0.2, inplace=True),
            spectral_norm(nn.Conv2d(256, 512, kernel_size=3, stride=2, padding=1, bias=False)),  # 13->7
            nn.BatchNorm2d(512),
            nn.LeakyReLU(0.2, inplace=True),
        )

        self.to_latent = spectral_norm(
            nn.Conv2d(512, latent_dim, kernel_size=7, stride=1, padding=0, bias=True)
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through spectrally normalized encoder.
...
class SNEncoder2D_GN(nn.Module):
    """Spectrally normalized encoder with GroupNorm instead of BatchNorm.

    BatchNorm uses different statistics in train vs eval mode (batch stats vs
    running stats), which introduces a train/val gap independent of overfitting.
    GroupNorm computes per-sample, per-group statistics — identical behaviour
    in train and eval — removing this artificial gap.

    GroupNorm(num_groups=8, num_channels=C) divides C channels into 8 groups
    and normalises within each group. Works well for C >= 64.

    Architecture: identical to SNEncoder2D but BN -> GN.
    - Input   [100x100]
    - SN-Conv1 + GN(8,64)   [50x50]
    - SN-Conv2 + GN(8,128)  [25x25]
    - SN-Conv3 + GN(8,256)  [13x13]
    - SN-Conv4 + GN(8,512)  [7x7]
    - SN-Conv5 (to_latent)  [1x1]

    Args:
        latent_dim: Dimension of the latent space.
        design_shape: Original design shape (H, W) for reference.
        resize_dimensions: Dimensions to resize input to before encoding.
        gn_groups: Number of groups for GroupNorm (must divide each channel count).
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through SN + GroupNorm encoder.
...
class TrueSNDecoder2D(nn.Module):
    ...
    def forward(self, z: torch.Tensor, cond: torch.Tensor | None = None) -> torch.Tensor:
        ...
        Args:
            ...
            cond: Condition embedding (B, cond_dim) or None.
        ...
        x = self.deconv(x)
        x = self.resize_out(x)
        return torch.sigmoid(x * self.lipschitz_scale)
...
class SNMLPPredictor(nn.Module):
    ...
    Args:
        ...
        lipschitz_scale: Scales hidden layer activations before final layer.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        h = self.hidden(x)
        return self.output(h * self.lipschitz_scale)
