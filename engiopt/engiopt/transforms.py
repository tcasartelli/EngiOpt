...
def get_performance_target(problem: Problem, dataset: Dataset) -> th.Tensor:
    ...
    """
    all_obj_keys = [name for name, _ in problem.objectives]
    dataset_cols = set(dataset.column_names)
    obj_keys = [k for k in all_obj_keys if k in dataset_cols]
    skipped = set(all_obj_keys) - set(obj_keys)
    if skipped:
        print(f"[get_performance_target] WARNING: skipping objectives not found in dataset: {skipped}")

    if len(obj_keys) == 0:
        raise ValueError(f"None of the problem objectives {all_obj_keys} exist as columns in the dataset.")
