...
from engiopt.vanilla_lvae.utils import augment_designs, filter_dataset_by_condition
import wandb
...
class Args:
    ...
    """WandB run name. If None, auto-generates from problem_id, algo, seed, and timestamp."""
    seed: int = 1
...
if __name__ == "__main__":
    ...
    for epoch in range(args.n_epochs):
        ...
        for i, batch in enumerate(bar):
            ...
            if args.track:
                ...
                if batches_done % args.sample_interval == 0:
                    with th.no_grad():
                        # Encode in batches to avoid OOM with large/augmented datasets
                        z_list = []
                        for b in range(0, len(x_train), args.batch_size):
                            z_list.append(lvae.encode(x_train[b:b + args.batch_size].to(device)))
                        z = th.cat(z_list, dim=0)
                        z_std, idx = th.sort(z.std(0), descending=True)
                        z_mean = z.mean(0)
                        n_active = (z_std > 0).sum().item()

                        # Generate interpolated designs (first 25 samples only)
                        z25 = z[:25]
                        x_ints = []
                        for alpha in [0, 0.25, 0.5, 0.75, 1]:
                            z_ = (1 - alpha) * z25 + alpha * th.roll(z25, -1, 0)
        ...
        if args.save_model and epoch == args.n_epochs - 1:
            ...
            ckpt_path = os.path.join(args.checkpoint_dir, "constrained_vanilla_lvae.pth")
