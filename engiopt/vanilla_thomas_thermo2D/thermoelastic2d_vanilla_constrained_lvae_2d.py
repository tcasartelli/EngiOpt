...
Mirrors constrained_vanilla_lvae_2d.py exactly — only differences are:
- problem_id default set to thermoelastic2d
- wandb_project default set to thermo2d_baseline
- images_dir / checkpoint_dir added to Args to write outputs to $SCRATCH on Euler
...
@dataclass
class Args:
    """Command-line arguments for constrained LVAE training on thermoelastic2d."""
    ...
    wandb_project: str = "thermo2d_baseline"
    """WandB project name."""
    wandb_entity: str | None = None
    """WandB entity name. If None, uses the default entity."""
    wandb_run_name: str | None = None
    """WandB run name override. If None, auto-generated from problem_id, algo, seed, and timestamp."""
    seed: int = 1
    """Random seed for reproducibility.""" #which means in simple terms that the same data will be sampled in the same order, and the same initial weights will be generated for the model, allowing for consistent results across runs. if seed is 2 instead of 1, a different set of data samples and initial weights will be used, which may lead to different training dynamics and results.
    ...
    # Output dirs (on $SCRATCH for Euler cluster runs)
        ...
        for i, batch in enumerate(bar):
            ...
            if args.track:
                ...
                if batches_done % args.sample_interval == 0:
                    with th.no_grad():
                        xs = x_train.to(device) # Take from training set for visualisation of interpolation and latent space structure
                        z = lvae.encode(xs)
                        ...
                        # Validation interpolations
                        xs_val = x_val.to(device)
                        z_val = lvae.encode(xs_val)
                        x_ints_val = []
                        for alpha in [0, 0.25, 0.5, 0.75, 1]:
                            z_ = (1 - alpha) * z_val[:25] + alpha * th.roll(z_val, -1, 0)[:25]
                            x_ints_val.append(lvae.decode(z_).cpu().numpy())
                        xs_val_cpu = xs_val.cpu().numpy()
                    ...
                    # Plot 2: Interpolated designs on training data
                    ...
                    # Plot 4: Interpolated validation designs
                    fig, axs = plt.subplots(25, 6, figsize=(12, 25))
                    for i_row, j in product(range(25), range(5)):
                        axs[i_row, j + 1].imshow(x_ints_val[j][i_row].reshape(design_shape))
                        axs[i_row, j + 1].axis("off")
                        axs[i_row, j + 1].set_aspect("equal")
                    for ax, alpha in zip(axs[0, 1:], [0, 0.25, 0.5, 0.75, 1]):
                        ax.set_title(rf"$\alpha$ = {alpha}")
                    for i_row in range(25):
                        axs[i_row, 0].imshow(xs_val_cpu[i_row].reshape(design_shape))
                        axs[i_row, 0].axis("off")
                        axs[i_row, 0].set_aspect("equal")
                    axs[0, 0].set_title("groundtruth (val)")
                    fig.tight_layout()
                    val_interp_path = os.path.join(args.images_dir, f"val_interp_{batches_done}.png")
                    plt.savefig(val_interp_path)
                    plt.close()
                    ...
                    wandb.log(
                        {
                            ...
                            "val_interp_plot": wandb.Image(val_interp_path),
