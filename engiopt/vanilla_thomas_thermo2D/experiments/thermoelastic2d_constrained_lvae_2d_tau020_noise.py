"""Constrained LVAE for thermoelastic2D — τ=0.20 + latent noise σ=0.1.

Combines the two best-performing strategies found so far:
  - nmse_threshold = 0.20  (sweep sweet spot: best val NMSE 0.182 at 97/100 active dims)
  - latent_noise_sigma = 0.1  (decoder robustness; cLVAE val NMSE 0.188 at τ=0.05)

With τ=0.20 the volume pressure activates reliably (train NMSE reaches 0.088 easily)
and noise regularises the decoder against latent code imprecision. Together they
should compress more aggressively while keeping val NMSE at or below 0.182.
...
@dataclass
class Args:
    """Command-line arguments for constrained LVAE (τ=0.20 + noise) on thermoelastic2d."""
    ...
    latent_dim: int = 100
    nmse_threshold: float = 0.20
    """NMSE threshold for volume pressure activation. 0.20 is the sweep sweet spot."""
    ...
    """Gaussian noise σ added to z before decoding during training."""

    images_dir: str = os.path.join(
        os.environ.get("SCRATCH", "."),
        "thermoelastic2d_constrained_lvae_tau020_noise", "images",
    )
    checkpoint_dir: str = os.path.join(
        os.environ.get("SCRATCH", "."),
        "thermoelastic2d_constrained_lvae_tau020_noise", "checkpoints",
    )

    early_stopping: bool = True
    patience: int = 50
    min_delta: float = 0.001
    early_stopping_start_epoch: int = 2000
...
if __name__ == "__main__":
    ...
    print(f"\n{'=' * 60}")
    print("Constrained LVAE — thermoelastic2d  [τ=0.20 + noise σ=0.1]")
    print(f"Design shape     : {design_shape}")
    print(f"Latent dim       : {args.latent_dim}")
    print(f"NMSE threshold   : {args.nmse_threshold}  ← sweep sweet spot")
    print(f"Latent noise σ   : {args.latent_noise_sigma}")
    print(f"GN groups        : {args.gn_groups}")
    print(f"Early stopping   : from epoch {args.early_stopping_start_epoch}, patience {args.patience}")
    ...
    for epoch in range(args.n_epochs):
        ...
        if args.save_model and epoch == args.n_epochs - 1:
            ckpt_path = os.path.join(
                args.checkpoint_dir,
                "thermoelastic2d_constrained_lvae_tau020_noise.pth",
            )
            th.save({
                "epoch"    : epoch,
